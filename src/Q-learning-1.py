...
exec(open('data cleaning.py').read())
...
def qLearning(initial_state, num_episodes, discount_factor, alpha, epsilon):
    ...
    for ith_episode in range(num_episodes):
        ...
        for t in range(0, 100):
            ...
            if action == 1:
                col_sample = max(min_col_sample, (col_sample - 0.1))
            elif action == 2:
                col_sample = min(max_col_sample, (col_sample + 0.1))
            elif action == 3:
                row_sample = max(min_row_sample, (row_sample - 0.1))
            elif action == 4:
                row_sample = min(max_row_sample, (row_sample + 0.1))
            elif action == 5:
                g = max(min_g, (g-5))
            elif action == 6 :
                g = min(max_g, (g + 5))
            elif action == 7:
                eta = max(min_eta, (eta - 0.1))
            elif action == 8:
                eta = min(max_eta, (eta + 0.1))
            elif action == 9 :
                d = max(min_d, (d-1))
            elif action == 10:
                d = min(max_d, (d + 1))
            ...
            if ((col_sample > max_col_sample or col_sample < min_col_sample
                    ...
                    or c > max_c or c < min_c) and (t > 50)):
                break
            else:
                # take action
                ...
                max_dev_accuracy = max(dev_accuracy, max_dev_accuracy)
                print("dec accuracy = {0}, max = {1}".format(dev_accuracy, max_dev_accuracy))
                ...
                # Update statistics
                stats.episode_rewards[ith_episode] = max(reward, stats.episode_rewards[ith_episode])
    ...
    return Q, stats, max_dev_accuracy

#Q, stats = qLearning((1, 0.9, 0, 0.3, 6, 1), 10, discount_factor = 0.9, alpha = 0.6, epsilon = 0.1)
Q, stats, max_result = qLearning((1, 0.9, 0, 0.3, 6, 1), 10, discount_factor = 0.95, alpha = 0.9, epsilon = 0.2)
